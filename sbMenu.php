<!-- MENU SIDEBAR-->
<aside class="menu-sidebar d-none d-lg-block">
    <div class="logo">
        <a href="index.php">
            <img src="images/rsz_11pzm.jpg" alt="Cool Admin" />
        </a>
    </div>
    <div class="menu-sidebar__content js-scrollbar1">
        <nav class="navbar-sidebar">
            <ul class="list-unstyled navbar__list">
                <!-- Admin Menu -->
                <li class="has-sub">
                    <a class="js-arrow" href="#">
                        <i class="fas fa-desktop"></i>Admin</a>
                    <ul class="list-unstyled navbar__sub-list js-sub-list">
                        <li>
                            <a href="asetAdmin.php">Aset</a>
                        </li>
                        <li>
                            <a href="stafAdmin.php">Staf</a>
                        </li>
                        <li>
                            <a href="bookingAdmin.php">Pinjaman Aset</a>
                        </li>
                        <li>
                            <a href="rekod_pemulangan.php">Rekod Pemulangan</a>
                        </li>
                        <li>
                            <a href="report.php">Laporan</a>
                        </li>
                    </ul>
                </li>
                
                <!-- Staf Menu -->
                <li class="has-sub">
                    <a class="js-arrow" href="#">
                        <i class="fas fa-user"></i>Staf</a>
                    <ul class="list-unstyled navbar__sub-list js-sub-list">
                        <li>
                            <a href="profil_saya.php">Profil Saya</a>
                        </li>
                        <li>
                            <a href="booking_form.php">Permohonan Baru</a>
                        </li>
                        <li>
                            <a href="list_aset.php">Aset</a>
                        </li>
                        <li>
                            <a href="booking_list.php">Senarai Permohonan</a>
                        </li>
                    </ul>
                </li>
            </ul>
        </nav>
    </div>
</aside>
<!-- END MENU SIDEBAR-->